...
### Added
- `Engine::set_compile_hook`, `set_instantiate_hook`, `set_trap_hook`, `set_fuel_exhausted_hook` and `set_memory_growth_hook`, with a `clear_*` for each, wrapping the five engine events zwasm 2.7.0 added. They are what lets an embedder observe a guest without instrumenting it — a shim or a server feeding its own metrics. Each is an event rather than a duration: zwasm's core carries no clock, so a hook reports that something happened and the embedder times it. An `instance_id` of 0 means "no instance" in the C ABI and arrives here as `None`, which is what separates a guest's `memory.grow` from a host growing a `Memory::new` it made itself. The trap hook's message is borrowed for the call and is not NUL-terminated, so the closure takes a `&str` and has to copy to keep it. Calling back into the engine from inside a hook is undefined — the hook fires in the middle of the operation it reports — and a panic in a hook is swallowed, because the C callback returns `void` and there is nowhere to turn one into a trap the way `Func::new` does. Three measured surprises are documented on the methods: a trap the embedder minted with `wasm_trap_new` is not reported, so an `Err` or a panic out of a `Func::new` closure never reaches the trap hook; a failed instantiation raises nothing but still consumes an id, so ids are not dense; and a start function that traps reports a trap carrying an id the instantiate hook never emits (#34)
- `Func::new`, which takes a Rust closure instead of a raw C callback, and `ValType` for declaring its signature. `Func::new_host` stays as the escape hatch for an embedder that already has an `extern "C"` function. The closure is `Fn` rather than `FnMut`, because a guest can reach the same import re-entrantly, and needs neither `Send` nor `Sync`, because nothing here can move a store to another thread — so it can capture an `Rc`. An `Err` from it becomes a trap, and so does a panic, which would otherwise abort the process on its way out of `extern "C"`. So does a result of the wrong type, or a count that does not match the slots zwasm supplied — both refused rather than passed on to the guest. It cannot read the caller's memory: zwasm's callback carries no instance to resolve an export against, asked upstream as zwasm/zwasm#486. With this, all three traits `wasmi-benchmarks` asks of a runtime are reachable (zwasm/zwasm#296)
- `Module::imports`, which reports a module's import section — module name, field name and kind — in declaration order. `Instance::new` takes its imports by position, which only works for a caller who wrote the module; this is what lets one who did not order its own functions by name, and is what `wasmi-benchmarks` needs for `link_func` (zwasm/zwasm#296). The names are owned rather than borrowed from the module, unlike wasmtime's, because they live in a C vector the call has to free before returning. A name that is not valid UTF-8 arrives as U+FFFD: the binary format rules such a name out, but zwasm decodes it anyway (zwasm/zwasm#474). The list can also be shorter than the module's import section — zwasm drops tag imports, and an allocation failure inside it drops an entry — with no status for either, so the doc says what cannot be detected from here (zwasm/zwasm#475)
